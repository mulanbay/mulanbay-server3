...
public class FamilyService extends BaseHibernateDao {
    ...
    public FamilyUser getAdminFamilyUser(Long userId) {
        try {
            String hql = "from FamilyUser where admin=1 and userId=?1 and status=?2 ";
            FamilyUser fu = this.getEntity(hql,FamilyUser.class, userId, FamilyUserStatus.PASSED);
            return fu;
    ...
    public FamilyUser getFamilyUser(Long userId) {
        try {
            String hql = "from FamilyUser where userId=?1 ";
            FamilyUser fu = this.getEntity(hql,FamilyUser.class, userId);
            return fu;
        } catch (BaseException e) {
            throw new PersistentException(ErrorCode.OBJECT_GET_LIST_ERROR,
                    "获取身家庭用户异常", e);
    ...
    public void dismissFamily(Long familyId) {
        try {
            //删除成员
            String hql = "delete from FamilyUser where familyId=?1 ";
            this.updateEntities(hql, familyId);
            //删除家庭
            String hql2 = "delete from Family where id=?1 ";
    ...
    public List<FamilyUser> getFamilyUserList(Long familyId, FamilyUserStatus status) {
        try {
            String hql = "from FamilyUser where familyId=?1 ";
            if (status != null) {
                hql += "and status=" + status.ordinal();
            }
            return this.getEntityListHI(hql,NO_PAGE,NO_PAGE_SIZE,FamilyUser.class, familyId);
        } catch (BaseException e) {
            throw new PersistentException(ErrorCode.OBJECT_GET_LIST_ERROR,
                    "获取家庭用户列表异常", e);
        }
