...
import cn.mulanbay.pms.web.bean.req.data.calendar.CalendarTemplateForm;
import cn.mulanbay.pms.web.bean.req.data.calendar.CalendarTemplateSH;
...
public class CalendarTemplateController extends BaseController {

    private static Class<CalendarTemplate> beanClass = CalendarTemplate.class;
    ...
    public ResultBean userTree(ReportUserTreeSH sf) {
        List<TreeBean> list = this.createTree(sf.getLevel(),sf.getBussType());
    ...
    public ResultBean tree(ReportTreeSH sf) {
        List<TreeBean> list = this.createTree(null,sf.getBussType());
        return callback(list);
    }


    private List<TreeBean> createTree(Integer maxLevel, BussType bussType) {
        CalendarTemplateSH sh = new CalendarTemplateSH();
        ...
        pr.addSort(s);
        pr.addSort(s2);
        List<CalendarTemplate> gtList = baseService.getBeanList(pr);
        ...
        int n = gtList.size();
        for (int i = 0; i < n; i++) {
            CalendarTemplate pc = gtList.get(i);
    ...
    public ResultBean list(CalendarTemplateSH sf) {
        PageRequest pr = sf.buildQuery();
        pr.setBeanClass(beanClass);
        Sort s = new Sort("orderIndex", Sort.ASC);
        pr.addSort(s);
        PageResult<CalendarTemplate> qr = baseService.getBeanResult(pr);
    ...
    public ResultBean create(@RequestBody @Valid CalendarTemplateForm form) {
        CalendarTemplate bean = new CalendarTemplate();
        ...
        if(fromTemplateId!=null&&form.getCopyItems()!=null&&form.getCopyItems()==true){
            //加载配置项
            List<StatBindConfig> copyConfigList = statBindConfigService.getConfigList(fromTemplateId, StatBussType.CALENDAR);
        ...
        userCalendarService.saveCalendarTemplate(bean,configList);
    ...
    public ResultBean get(@RequestParam(name = "templateId") Long templateId) {
        CalendarTemplate bean = baseService.getObject(beanClass, templateId);
    ...
    public ResultBean edit(@RequestBody @Valid CalendarTemplateForm form) {
        CalendarTemplate bean = baseService.getObject(beanClass, form.getTemplateId());
        BeanCopy.copy(form, bean);
    ...
    public ResultBean delete(@RequestBody @Valid CommonDeleteForm deleteRequest) {
        ...
        for(String s :ss){
            userCalendarService.deleteCalendarTemplate(Long.valueOf(s));
        }
        return callback(null);
    }
